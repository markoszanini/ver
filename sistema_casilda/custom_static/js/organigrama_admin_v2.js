document.addEventListener('DOMContentLoaded', function() {
    ...
    function refreshSubdivisiones() {
        updateOptionsCustom($subdivision, '/organigrama/ajax/get_subdivisiones/', { area_id: $area.val(), depto_id: $departamento.val(), div_id: $division.val() }); 
    }
    function refreshSecciones() {
        updateOptionsCustom($seccion, '/organigrama/ajax/get_secciones/', { area_id: $area.val(), depto_id: $departamento.val(), subdiv_id: $subdivision.val() });
    }
    ...
    $area.on('change', function() {
        console.log("Area changed:", $area.val());
        refreshDirecciones();         
    ...
    $direccion.on('change', function() {
        console.log("Direccion changed:", $direccion.val());
        refreshDepartamentos();       
        refreshDivisiones();
        refreshSubdivisiones();
        refreshSecciones();
    ...
    $departamento.on('change', function() {
        console.log("Departamento changed:", $departamento.val());
        refreshDivisiones();          
        refreshSubdivisiones();
        refreshSecciones();           
        refreshOficinas();            
    ...
    // EJECUCIÓN INICIAL (Fix para formularios de edición)
    // No forzamos refrescos masivos al inicio para evitar bucles si Select2 dispara eventos
    // pero nos aseguramos que las oficinas se refresquen al menos una vez.
    setTimeout(refreshOficinas, 500);

    console.log("Organigrama Admin JS loaded with full cascade logic.");
});
